...
void ShoppingCart::loadShoppingCart(const QString& username) {
    QList<ShoppingCart*> allCarts = FileManager::loadShoppingCarts();
    for(int i = 0;i < allCarts.size(); i++) {
        if(allCarts[i]->getConsumerUsername() == username) {
            items.clear();
            items = allCarts[i]->getItems();
        }
    }
    emit cartChanged();
}
...
void ShoppingCart::saveShoppingCart(const QString& username) {
    ShoppingCart* shoppingCart = new ShoppingCart(username, items);
    FileManager::saveShoppingCart(shoppingCart);
}
